...
napi_value FileNapi::New(napi_env env, napi_callback_info info)
{
    ...
    if(value != nullptr) {
        ASSERT_ERR(ctxt->env, type == UD_TYPE_MAP.at(UDType::FILE), Status::E_ERROR, "invalid arguments!");
        file->value_ = std::static_pointer_cast<File>(UnifiedRecordNapi::GetNativeRecord(ctxt->env, type, value));
    } else {
