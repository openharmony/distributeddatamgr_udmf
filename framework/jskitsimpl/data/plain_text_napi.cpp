...
napi_value PlainTextNapi::New(napi_env env, napi_callback_info info)
{
    ...
    if(value != nullptr) {
        ASSERT_ERR(ctxt->env, type == UD_TYPE_MAP.at(UDType::PLAIN_TEXT), Status::E_ERROR, "invalid arguments!");
        plainText->value_ = std::static_pointer_cast<PlainText>(UnifiedRecordNapi::GetNativeRecord(ctxt->env, type, value));
    } else {
