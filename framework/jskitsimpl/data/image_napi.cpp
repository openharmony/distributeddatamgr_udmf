...
napi_value ImageNapi::Constructor(napi_env env)
{
    napi_property_descriptor properties[] = {
        ...
        /* Image property */
        DECLARE_NAPI_GETTER_SETTER("imageUri", GetUri, SetUri),
    };
...
ImageNapi *ImageNapi::GetImage(napi_env env, napi_callback_info info, std::shared_ptr<ContextBase> ctxt)
{
    ctxt->GetCbInfoSync(env, info);
    ASSERT_ERR(ctxt->env, ctxt->status == napi_ok, Status::E_INVALID_PARAMETERS, "invalid arguments!");
    return reinterpret_cast<ImageNapi *>(ctxt->native);
}

napi_value ImageNapi::GetUri(napi_env env, napi_callback_info info)
{
...
napi_value ImageNapi::SetUri(napi_env env, napi_callback_info info)
{
    ...
    ASSERT_ERR(ctxt->env, ctxt->status == napi_ok, Status::E_INVALID_PARAMETERS, "invalid arguments!");
    auto image = reinterpret_cast<ImageNapi *>(ctxt->native);
    ASSERT_ERR(
        ctxt->env, (image != nullptr && image->value_ != nullptr), Status::E_INVALID_PARAMETERS, "invalid object!");
