...
napi_value ApplicationDefinedRecordNapi::New(napi_env env, napi_callback_info info)
{
    ...
    if(value != nullptr) {
        ASSERT_ERR(ctxt->env, type == UD_TYPE_MAP.at(UDType::APPLICATION_DEFINED_RECORD), Status::E_ERROR, "invalid arguments!");
        record->value_ = std::static_pointer_cast<ApplicationDefinedRecord>(UnifiedRecordNapi::GetNativeRecord(ctxt->env, type, value));
    } else {
