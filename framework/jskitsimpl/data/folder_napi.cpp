...
napi_value FolderNapi::New(napi_env env, napi_callback_info info)
{
    ...
    if(value != nullptr) {
        ASSERT_ERR(ctxt->env, type == UD_TYPE_MAP.at(UDType::FOLDER), Status::E_ERROR, "invalid arguments!");
        folder->value_ = std::static_pointer_cast<Folder>(UnifiedRecordNapi::GetNativeRecord(ctxt->env, type, value));
    } else {
