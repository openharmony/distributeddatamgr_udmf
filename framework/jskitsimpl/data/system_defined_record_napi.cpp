...
napi_value SystemDefinedRecordNapi::SetDetails(napi_env env, napi_callback_info info)
{
    ...
    auto input = [env, ctxt, &details](size_t argc, napi_value *argv) {
        ...
        ASSERT_BUSINESS_ERR(ctxt, ctxt->status == napi_ok, Status::E_INVALID_PARAMETERS, "Parameter error: parameter details type must be Record<string, number | string | Uint8Array>");
    };
