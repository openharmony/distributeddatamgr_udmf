...
/**
 * @tc.name: GetTypes_001
 * @tc.desc: Normal test of GetTypes
 * @tc.type: FUNC
 */
HWTEST_F(UnifiedRecordTest, GetTypes_001, TestSize.Level0)
{
    UnifiedRecord record;

    auto entries = std::make_shared<std::map<std::string, ValueType>>();
    entries->insert({"type1", ENTITY});
    entries->insert({"type2", OBJECT});
    entries->insert({"", TEXT});
    entries->insert({"type3", PLAIN_TEXT});
    record.SetInnerEntries(entries);

    auto types = record.GetTypes();
    EXPECT_EQ(types.size(), 3);
    EXPECT_EQ(types[0], "type1");
    EXPECT_EQ(types[1], "type2");
    EXPECT_EQ(types[2], "type3");
}

/**
 * @tc.name: GetTypes_002
 * @tc.desc: Normal test of GetTypes
 * @tc.type: FUNC
 */
HWTEST_F(UnifiedRecordTest, GetTypes_002, TestSize.Level0)
{
    ...
    EXPECT_EQ(types[1], "type2");
    EXPECT_EQ(types[2], "type3");
    EXPECT_EQ(types[3], "general.html");
}
} // OHOS::Test