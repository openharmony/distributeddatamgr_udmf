...
* @tc.desc: ParseStoredCustomUtdJson, empty JsonData
* @tc.type: FUNC
*/
HWTEST_F(CustomUtdStoreTest, ParseStoredCustomUtdJson001, TestSize.Level1)
{
    ...
    std::vector<TypeDescriptorCfg> typesCfg;
    CustomUtdJsonParser parser;
    std::string emptyJsonData = "";
    bool ret = parser.ParseStoredCustomUtdJson(emptyJsonData, typesCfg);
    EXPECT_EQ(ret, false);
...
* @tc.desc: ParseStoredCustomUtdJson, invalid JsonData
* @tc.type: FUNC
*/
HWTEST_F(CustomUtdStoreTest, ParseStoredCustomUtdJson002, TestSize.Level1)
{
    ...
    std::vector<TypeDescriptorCfg> typesCfg;
    CustomUtdJsonParser parser;
    std::string invalidJsonData = "{ invalid json }\0";
    bool ret = parser.ParseStoredCustomUtdJson(invalidJsonData, typesCfg);
    EXPECT_EQ(ret, true);
    EXPECT_EQ(typesCfg.empty(), true);

    invalidJsonData = R"({ ["item1", "item2", "item3"] })";
    ret = parser.ParseStoredCustomUtdJson(invalidJsonData, typesCfg);
    EXPECT_EQ(ret, true);
    EXPECT_EQ(typesCfg.empty(), true);
...
* @tc.desc: ParseStoredCustomUtdJson, JsonData is valid but no data is needed
* @tc.type: FUNC
*/
HWTEST_F(CustomUtdStoreTest, ParseStoredCustomUtdJson003, TestSize.Level1)
{
    ...
    bool ret = parser.ParseStoredCustomUtdJson(validJsonData, typesCfg);
    EXPECT_EQ(ret, true);
    EXPECT_EQ(typesCfg.empty(), true);

    validJsonData = R"({"CustomUTDs": [{}]})";
    ret = parser.ParseStoredCustomUtdJson(validJsonData, typesCfg);
    EXPECT_EQ(ret, true);
    EXPECT_EQ(typesCfg.empty(), true);
