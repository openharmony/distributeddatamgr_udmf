...
* @tc.desc: Set multiple record with valid params and async get data
* @tc.type: FUNC
*/
HWTEST_F(UdmfClientTest, GetDataAsync01, TestSize.Level1)
{
    ...
    auto callback = [this, inputRecords](ProgressInfo progress, UnifiedData &outputData) {
        LOG_INFO(UDMF_TEST, "GetDataAsync01 callback begin status=%{public}u, progress=%{public}u, name=%{public}s.",
            progress.status, progress.progress, progress.srcDevName.c_str());
        if (progress.status == ASYNC_IDLE || progress.status == ASYNC_RUNNING) {
            return;
        }
        ASSERT_EQ(progress.status, ASYNC_SUCCESS);
