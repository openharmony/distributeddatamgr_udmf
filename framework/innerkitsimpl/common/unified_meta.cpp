...
static constexpr UtdType UTD_TYPES[] = {
    ...
    { KINSOFT_OFFICE, "KINSOFT_OFFICE", "com.kingsoft.office	general.zip-archive" },
    { KINSOFT_WRITER_WPS, "KINSOFT_WRITER_WPS", "com.kingsoft.office.writer.wps" },
    { KINSOFT_WRITER_WPT, "KINSOFT_WRITER_WPT", "com.kingsoft.office.writer.wpt" },
...
bool Object::GetValue(const std::string &key, std::string &value)
{
    if (value_.find(key) != value_.end()) {
        if(std::holds_alternative<std::string>(value_[key])) {
            value = std::get<std::string>(value_[key]);
            return true;
        }
    }
    return false;
...
{
    if (value_.find(key) != value_.end()) {
        if(std::holds_alternative<std::shared_ptr<Object>>(value_[key])) {
            value = std::get<std::shared_ptr<Object>>(value_[key]);
            return true;
        }
    }
    return false;
