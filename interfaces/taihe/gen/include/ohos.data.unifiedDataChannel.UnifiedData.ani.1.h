...
inline ::ohos::data::unifiedDataChannel::UnifiedData ohos_data_unifiedDataChannel_UnifiedData_fromANI(ani_env* env, ani_object ani_obj) {
    struct cpp_impl_t {
        ...
        void AddRecord1(::ohos::data::unifiedDataChannel::AllRecords const& cpp_arg_unifiedRecord) {
            ::taihe::env_guard guard;
            ani_env *env = guard.get_env();
            ani_ref ani_arg_unifiedRecord = ohos_data_unifiedDataChannel_AllRecords_intoANI(env, cpp_arg_unifiedRecord);
            env->Object_CallMethod_Void(static_cast<ani_object>(this->ref), TH_ANI_FIND_CLASS_METHOD(env, "L@ohos/data/unifiedDataChannel/unifiedDataChannel/UnifiedData;", "addRecord1", nullptr), ani_arg_unifiedRecord);
        }
        ::taihe::array<::ohos::data::unifiedDataChannel::AllRecords> GetRecords1() {
            ::taihe::env_guard guard;
            ani_env *env = guard.get_env();
            ani_array_ref ani_result;
            env->Object_CallMethod_Ref(static_cast<ani_object>(this->ref), TH_ANI_FIND_CLASS_METHOD(env, "L@ohos/data/unifiedDataChannel/unifiedDataChannel/UnifiedData;", "getRecords1", nullptr), reinterpret_cast<ani_ref*>(&ani_result));
