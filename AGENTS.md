...
| 命令 | 用途 | 备注 |
|---|---|---|
| `./build.sh --product-name <product> --build-target udmf` | 构建 UDMF 包 | 根 `BUILD.gn` 目标 |
| `./build.sh --product-name <product> --build-target udmf_test` | 构建全部单元测试、fuzz 测试 | 聚合 common、InnerKit、JSKit、NDK 测试、fuzz 测试 |
...
| 门禁 | 本地等价命令 |
|---|---|
| 包构建 | `./build.sh --product-name <product> --build-target udmf` |
| 单元测试、Fuzz 测试 | `./build.sh --product-name <product> --build-target udmf_test` |
